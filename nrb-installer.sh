...
install_skyport() {
    ...
    if [[ -e "$install_dir" ]]; then
        warn "Skyport directory already exists: $install_dir"
        read -rp "Update existing repository instead? [y/N]: " answer
        if [[ "$answer" =~ ^[Yy]$ ]]; then
            if git -C "$install_dir" pull --ff-only; then
                success "Skyport repository updated."
            else
                error "Skyport update failed."
            fi
        else
            info "Skyport installation cancelled."
            pause
            return
        fi
    else
        info "Cloning Skyport Panel..."
        if git clone "$SKYPORT_REPO" "$install_dir"; then
            success "Skyport source downloaded to $install_dir."
        else
    ...
    # Install Node.js/npm only if not present. Do not silently install
    # arbitrary third-party Node packages.
    if ! command_exists node || ! command_exists npm; then
        info "Node.js/npm is required by Skyport. Installing distribution packages..."
        run_as_root apt-get install -y nodejs npm || {
            error "Could not install Node.js/npm."
            pause
            return
        }
    fi
    ...
    if [[ -f "$install_dir/package.json" ]]; then
        info "Installing Skyport Node dependencies..."
        if (cd "$install_dir" && npm install); then
            success "Skyport Node dependencies installed."
        else
            error "Skyport npm dependency installation failed."
            pause
            return
        fi
    else
        warn "No package.json found in the cloned repository."
        warn "The repository may require a different build/start procedure."
    fi

    cat <<EOF

${CYAN}Skyport source:${NC} $install_dir
${CYAN}Repository:${NC} $SKYPORT_REPO

The repository may require project-specific environment variables,
database configuration, Docker, and a production process manager.
This installer does not invent those settings.
EOF
...
repair_skyport() {
    ...
    if [[ ! -d "$dir/.git" ]]; then
        error "Skyport Git repository not found at $dir."
        return 1
    fi
...
repair_nokvm() {
    download_and_run_script "$NOKVM_URL" "nrb-nokvm-repair"
}
