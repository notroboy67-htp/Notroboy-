...
SKYPORTD_REPO="https://github.com/skyport-team/skyportd.git"
...
install_skyport() {
    banner
    printf '%b\n\n' "${WHITE}SKYPORT PANEL + SKYPORTD INSTALLATION${NC}"
    ...
    local panel_dir="/etc/skyport"
    local daemon_dir="/etc/skyportd"

    # ----------------------------- Docker -----------------------------------
    info "[1/8] Installing/checking Docker..."
    if ! command_exists docker; then
        info "Docker is not installed. Installing Docker Engine..."
        if ! curl -fsSL https://get.docker.com/ | CHANNEL=stable run_as_root bash; then
    ...
    run_as_root systemctl enable docker.service 2>/dev/null || true
    ...
    # ----------------------------- Node.js ----------------------------------
    info "[2/8] Installing/checking Node.js 22..."
    local node_major=""
    ...
    if [[ "$node_major" != "22" ]]; then
        ...
        local node_key_tmp="${TMP_DIR}/nodesource.gpg"
        if ! curl -fsSL \
            https://deb.nodesource.com/gpgkey/nodesource-repo.gpg.key \
            | gpg --dearmor > "$node_key_tmp"; then
            error "Could not download NodeSource signing key."
            pause
            return
        fi

        run_as_root install -m 0644 "$node_key_tmp" /etc/apt/keyrings/nodesource.gpg
        printf '%s\n' \
            'deb [signed-by=/etc/apt/keyrings/nodesource.gpg] https://deb.nodesource.com/node_22.x nodistro main' \
            | run_as_root tee /etc/apt/sources.list.d/nodesource.list >/dev/null

        run_as_root apt-get update || {
            error "NodeSource apt update failed."
            pause
            return
        }

        run_as_root apt-get install -y nodejs || {
            error "Node.js 22 installation failed."
            pause
            return
        }
    fi
    ...
    # ----------------------------- Panel -----------------------------------
    info "[3/8] Installing Skyport Panel..."
    if [[ -d "$panel_dir/.git" ]]; then
        info "Existing Skyport Panel repository found."
        read -rp "Update existing Panel repository? [y/N]: " answer
        if [[ "$answer" =~ ^[Yy]$ ]]; then
            git -C "$panel_dir" pull --ff-only || {
                error "Skyport Panel update failed."
                pause
                return
            }
        fi
    elif [[ -e "$panel_dir" ]]; then
        error "$panel_dir exists but is not a Git repository."
        pause
        return
    else
        run_as_root mkdir -p /etc
        run_as_root git clone "$SKYPORT_REPO" "$panel_dir" || {
            error "Skyport Panel clone failed."
            pause
            return
        }
    fi
    ...
    [[ -f "$panel_dir/package.json" ]] || {
        error "Skyport Panel package.json not found."
        pause
        return
    }

    if ! (cd "$panel_dir" && npm install); then
        error "Skyport Panel npm install failed."
        pause
        return
    fi

    if [[ ! -f "$panel_dir/config.json" ]]; then
        if [[ -f "$panel_dir/example_config.json" ]]; then
            run_as_root cp "$panel_dir/example_config.json" "$panel_dir/config.json"
            success "Panel config.json created from example_config.json."
        else
            warn "Panel example_config.json was not found."
            warn "The current repository revision may use a different configuration method."
        fi
    fi

    success "Skyport Panel installed."

    # ----------------------------- Daemon ----------------------------------
    info "[4/8] Installing Skyport Daemon..."
    if [[ -d "$daemon_dir/.git" ]]; then
        info "Existing Skyport Daemon repository found."
        read -rp "Update existing Daemon repository? [y/N]: " answer
        if [[ "$answer" =~ ^[Yy]$ ]]; then
            git -C "$daemon_dir" pull --ff-only || {
                error "Skyport Daemon update failed."
                pause
                return
            }
        fi
    elif [[ -e "$daemon_dir" ]]; then
        error "$daemon_dir exists but is not a Git repository."
        pause
        return
    else
        run_as_root mkdir -p /etc
        run_as_root git clone "$SKYPORTD_REPO" "$daemon_dir" || {
            error "Skyport Daemon clone failed."
            pause
            return
        }
    fi

    [[ -f "$daemon_dir/package.json" ]] || {
        error "Skyport Daemon package.json not found."
        pause
        return
    }

    if ! (cd "$daemon_dir" && npm install); then
        error "Skyport Daemon npm install failed."
        pause
        return
    fi

    if [[ ! -f "$daemon_dir/config.json" ]]; then
        if [[ -f "$daemon_dir/example_config.json" ]]; then
            run_as_root cp "$daemon_dir/example_config.json" "$daemon_dir/config.json"
            success "Daemon config.json created from example_config.json."
        else
            warn "Daemon example_config.json was not found."
            warn "Configure skyportd according to the repository documentation."
        fi
    fi

    success "Skyport Daemon installed."

    # ------------------------------- PM2 -----------------------------------
    info "[5/8] Installing PM2..."
    ...
    success "PM2 is available."

    # ----------------------------- Configure -------------------------------
    info "[6/8] Checking configuration..."
    if [[ ! -f "$panel_dir/config.json" ]]; then
        warn "Panel config.json is missing."
    else
        success "Panel config.json exists."
    fi

    if [[ ! -f "$daemon_dir/config.json" ]]; then
        warn "Daemon config.json is missing."
    else
        success "Daemon config.json exists."
    fi

    echo
    warn "Skyport access keys/node configuration may require values from your Panel."
    info "The installer will not invent or overwrite those credentials."

    # ------------------------------- Start ---------------------------------
    info "[7/8] Starting Skyport services with PM2..."
    pm2 delete skyport 2>/dev/null || true
    pm2 delete skyportd 2>/dev/null || true

    if [[ -f "$panel_dir/index.js" ]]; then
        if ! (cd "$panel_dir" && pm2 start index.js --name skyport); then
            error "Could not start Skyport Panel."
            pm2 logs skyport --lines 50 --nostream 2>/dev/null || true
            pause
            return
        fi
    else
        error "Panel index.js was not found."
        error "The repository's startup entry point has changed."
        pause
        return
    fi

    if [[ -f "$daemon_dir/index.js" ]]; then
        if ! (cd "$daemon_dir" && pm2 start index.js --name skyportd); then
            error "Could not start Skyport Daemon."
            pm2 logs skyportd --lines 50 --nostream 2>/dev/null || true
            pause
            return
        fi
    else
        error "Daemon index.js was not found."
        error "The repository's startup entry point has changed."
        pm2 delete skyport 2>/dev/null || true
        pause
        return
    ...
    pm2 startup systemd -u root --hp /root >/tmp/skyport-pm2-startup.txt 2>&1 || true
    local startup_cmd=""
    startup_cmd="$(grep -E '^sudo |^env ' /tmp/skyport-pm2-startup.txt 2>/dev/null | tail -n 1 || true)"
    if [[ -n "$startup_cmd" ]]; then
        # Execute only the exact command printed by PM2; do not invent one.
        bash -c "$startup_cmd" 2>/dev/null || true
    fi
    pm2 save
    ...
    echo
    info "Verifying PM2 services..."
    pm2 status

    local panel_ok="no"
    local daemon_ok="no"
    if pm2 jlist 2>/dev/null | grep -q '"name":"skyport"'; then
        panel_ok="yes"
    fi
    if pm2 jlist 2>/dev/null | grep -q '"name":"skyportd"'; then
        daemon_ok="yes"
    fi
    ...
    echo
    printf '%b\n' "${GREEN}============================================================${NC}"
    printf '%b\n' "${GREEN}           SKYPORT INSTALLATION COMPLETE${NC}"
    printf '%b\n' "${GREEN}============================================================${NC}"
    echo
    echo "Panel directory : $panel_dir"
    echo "Daemon directory: $daemon_dir"
    echo "Panel port      : 3001 (verify config.json)"
    echo
    echo "Panel PM2       : $panel_ok"
    echo "Daemon PM2      : $daemon_ok"
    echo
    echo "Create admin user:"
    echo "  cd $panel_dir"
    echo "  npm link"
    echo "  npm run seed"
    echo "  npm run createUser"
    echo
    echo "Check services:"
    echo "  pm2 status"
    echo
    echo "Panel logs:"
    echo "  pm2 logs skyport"
    echo
    echo "Daemon logs:"
    echo "  pm2 logs skyportd"
    echo
    echo -e "${GREEN}Panel: http://${ip:-YOUR_SERVER_IP}:3001${NC}"
    echo
    warn "Configure Panel/skyportd access keys and add the node before using the daemon."
    pause
}
